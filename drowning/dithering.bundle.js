...
}`;function v(){let J=navigator.userAgent.toLowerCase();return J.includes("safari")&&!J.includes("chrome")&&!J.includes("android")}function x(J){let K=visualViewport?.scale??1,Q=visualViewport?.width??window.innerWidth,Y=window.innerWidth-J.documentElement.clientWidth,V=K*Q+Y,$=outerWidth/V,X=Math.round(100*$);if(X%5===0)return X/100;if(X===33)return 0.3333333333333333;if(X===67)return 0.6666666666666666;if(X===133)return 1.3333333333333333;return $}var A=`
...
`;function q(J){if(Array.isArray(J)){if(J.length===4)return J;if(J.length===3)return[...J,1];return L}if(typeof J!=="string")return L;let K,Q,Y,V=1;if(J.startsWith("#"))[K,Q,Y,V]=p(J);else if(J.startsWith("rgb"))[K,Q,Y,V]=S(J);else if(J.startsWith("hsl"))[K,Q,Y,V]=c(u(J));else return console.error("Unsupported color format",J),L;return[T(K,0,1),T(Q,0,1),T(Y,0,1),T(V,0,1)]}function p(J){if(J=J.replace(/^#/,""),J.length===3)J=J.split("").map(($)=>$+$).join("");if(J.length===6)J=J+"ff";let K=parseInt(J.slice(0,2),16)/255,Q=parseInt(J.slice(2,4),16)/255,Y=parseInt(J.slice(4,6),16)/255,V=parseInt(J.slice(6,8),16)/255;return[K,Q,Y,V]}function S(J){let K=J.match(/^rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([0-9.]+))?\s*\)$/i);if(!K)return[0,0,0,1];return[parseInt(K[1]??"0")/255,parseInt(K[2]??"0")/255,parseInt(K[3]??"0")/255,K[4]===void 0?1:parseFloat(K[4])]}function u(J){let K=J.match(/^hsla?\s*\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([0-9.]+))?\s*\)$/i);if(!K)return[0,0,0,1];return[parseInt(K[1]??"0"),parseInt(K[2]??"0"),parseInt(K[3]??"0"),K[4]===void 0?1:parseFloat(K[4])]}function c(J){let[K,Q,Y,V]=J,$=K/360,X=Q/100,Z=Y/100,O,I,B;if(Q===0)O=I=B=Z;else{let j=(W,E,_)=>{if(_<0)_+=1;if(_>1)_-=1;if(_<0.16666666666666666)return W+(E-W)*6*_;if(_<0.5)return E;if(_<0.6666666666666666)return W+(E-W)*(0.6666666666666666-_)*6;return W},H=Z<0.5?Z*(1+X):Z+X-Z*X,U=2*Z-H;O=j(U,H,$+0.3333333333333333),I=j(U,H,$),B=j(U,H,$-0.3333333333333333)}return[O,I,B,V]}var T=(J,K,Q)=>Math.min(Math.max(J,K),Q),L=[0,0,0,1];var P=document.getElementById("bg-dither"),b=document.getElementById("bg-overlay-dither");function N(){document.body.classList.add("fallback-visible")}function s(){new z(b,C,{u_colorBack:q("#00000000"),u_colorFront:q("#FFC8E9"),u_shape:2,u_type:4,u_pxSize:2,u_scale:0.6,u_fit:0,u_rotation:0,u_offsetX:0,u_offsetY:0,u_originX:0.5,u_originY:0.5,u_worldWidth:0,u_worldHeight:0},void 0,0.4)}if(P&&b){let K=function(){try{new z(P,D,{u_image:J,u_imageAspectRatio:J.naturalWidth/J.naturalHeight,u_colorFront:q("#00000000"),u_colorBack:q("#00000000"),u_colorHighlight:q("#00000000"),u_originalColors:!0,u_inverted:!1,u_type:4,u_pxSize:2,u_colorSteps:3,u_scale:1,u_fit:2,u_rotation:0,u_offsetX:0,u_offsetY:0,u_originX:0.5,u_originY:0.5,u_worldWidth:0,u_worldHeight:0},void 0,0),s(),requestAnimationFrame(()=>{document.body.classList.add("shader-loaded")})}catch(Q){N()}},J=new Image;if(J.crossOrigin="anonymous",J.src="../img/drowning-in-moonlight/sky.png",J.onerror=N,J.complete&&J.naturalWidth>0)K();else J.onload=K}else N();
